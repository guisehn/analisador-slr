...
function getFirstSet(grammar, symbol, history) {
  ...
  var firsts = (grammar.productionSet[symbol] || []).map(function (e) {
    var firsts = [];
    ...
    for (var i = 0; i < e.length; i++) {
      ...
      if (i + 1 < e.length) {
        firsts.push(_.filter(first, function (s) {
          return s.symbol !== '';
        }));
      } else {
        firsts.push(first);
    ...
    return _.flatten(firsts).map(function (first) {
      first.productions = [e];
      return first;
    });
  });
...
function mergeDuplicates(list) {
  ...
  return _.map(firstsBySymbol, function (firsts, symbol) {
    var productions = _(firsts).map(function (f) {
...
function getFirstSets(grammar) {
  return _.mapValues(grammar.productionSet, function (rightSide, leftSide) {
    return getFirstSet(grammar, leftSide);
  });
}
...
function getFollowSet(grammar, symbol, history) {
  ...
  _.forEach(grammar.productionSet, function (rightSide, leftSide) {
    _.forEach(rightSide, function (production) {
      ...
      positionsOfSymbol.forEach(function (symbolPosition) {
        for (var i = symbolPosition; i < production.length; i++) {
          ...
          followSet = followSet.concat(first.filter(function (s) {
            return s !== '';
          }));
...
function getFollowSets(grammar) {
  return _.mapValues(grammar.productionSet, function (rightSide, leftSide) {
    return getFollowSet(grammar, leftSide);
  });
}
...
function extractPairs(str) {
  return str.trim().split('\n').map(function (l) {
    ...
    return [l[0], l.slice(1).join(':').split('|')];
  });
}
...
function validateMissingNonTerminals(pairs) {
  _.forEach(pairs, function (pair, n) {
    n++;

    _.forEach(pair[1], function (symbols) {
      _.forEach(symbols, function (symbol) {
        if (Utils.isNonTerminal(symbol) && !_.find(pairs, function (pair) {
          return pair[0] === symbol;
        })) {
          throw new Error('Símbolo ' + symbol + ' referenciado no lado direito da' + ' linha ' + n + ' não encontrado nas regras de produções.');
        }
      });
    });
  });
...
function validatePairs(pairs) {
  _.forEach(pairs, function (pair, n) {
    ...
    if (pair[0].length > 1) {
      throw new Error('A gramática deve ser GLC.' + ' Múltiplos símbolos à esquerda foram encontrados na linha ' + n + '.');
    }
...
function pairsToObject(pairs) {
  return _(pairs).groupBy('0').mapValues(function (l) {
    return l[0][1];
  }).value();
}
...
function isLeftFactored(grammar) {
  return _.every(grammar.productionSet, function (rightSide, leftSide) {
    var firstSymbols = _.map(rightSide, function (e) {
      return e[0];
    });
...
function isLeftRecursive(grammar) {
  return _.some(grammar.productionSet, function (rightSide, leftSide) {
    return _.some(rightSide, function (e) {
      return e[0] === leftSide;
    });
  });
}
...
Grammar.prototype.getTerminals = function () {
  return _(this.productionSet).values().flatten().map(function (s) {
    return s.split('');
  }).flatten().filter(function (s) {
    return Utils.isTerminal(s);
  }).uniq().value();
};
...
Grammar.prototype.getRepresentation = function () {
  ...
  var productionSet = _.map(Utils.emptyToEpsilon(this.productionSet), function (right, left) {
...
$('#grammar-form').submit(function (e) {
  var grammar, error;
...
$('#sentence-recognizer-form').submit(function (e) {
  recognize();
  e.preventDefault();
});
...
function mountTable(object, leftTitle, rightTitle) {
  ...
  _.forEach(object, function (right, left) {
    var tr = $('<tr></tr>');
...
function showSentenceRecognition(recognition) {
  ...
  var message = $('<div role="alert" class="alert"></div>').addClass(recognition.success ? 'alert-success' : 'alert-danger').text(recognition.success ? 'A sentença foi reconhecida' : 'A sentença não foi reconhecida');
...
function emptyToEpsilon(object) {
  if (_.isArray(object)) {
    return object.map(function (e) {
      return emptyToEpsilon(e);
    });
  } else if (_.isObject(object)) {
    return _.mapValues(object, function (e) {
      return emptyToEpsilon(e);
    });
  } else {
