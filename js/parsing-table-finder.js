...
function generateTable(grammar, followSet) {
  ...
  // 2. Repita: Para todo I ∈ C e X ∈ G, calcular goto(I, X) e adicionar a C
  ...
  while (true) {
    ...
    for (var i in productionSetLastI) {
      ...
      var a = containsStateResult(c, productionSet)
      if (!a.symbol) {
        c['I' + (n++)] = productionSet
      }
    }
  ...
  for (var n in c) { // Para cada estado n de c
    ...
    for (var i in c[n]) { // Para cada produção de In
      ...
      if (x) {
        ...
        var resultStateResult = containsStateResult(c, resultClosure)
        var stateJ = resultStateResult['i'].replace('I', '')

        if (Utils.isTerminal(x)) {
          actionsTable[stateN][x] = 's' + stateJ
        } else {
          gotoTable[stateN][x] = parseInt(stateJ, 10)
        }
      } else {
        if (production.left !== '@') {
...
function closure(grammar, productionSet, index) {
  ...
  for (var i = index; i < productionSet.length; i++) {
    ...
    if (Utils.isNonTerminal(rightDotSymbol)) {
      ...
      var productionFound = getProduction(grammar, rightDotSymbol)

      // Faz a união entre as produções e as encontradas
      productionSet = _.union(productionSet, productionFound)
...
function containsStateResult(c, productionSet) {
  for (var i in c) {
